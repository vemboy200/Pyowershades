...
class PowerShadesConnection:
    ...
    async def async_request(
        ...
        async with self._lock:
            # Each attempt sends a fresh sequence, but replies are matched
            # by op only — the device does not echo the sequence on all ops.
...
async def async_discover_devices(
    ...
    """Discover PowerShades devices via UDP broadcast on all provided local addresses."""
    loop = asyncio.get_running_loop()
    ...
    for address in local_addresses:
        try:
            ...
            _LOGGER.debug("Could not bind discovery socket to %s: %s", address, err)
            continue
    ...
    _LOGGER.info("Discovery complete, found %d device(s)", len(results))
